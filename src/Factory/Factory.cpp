...
void Factory::makePlayer(std::function<void(Entity &e)> funcCreate, std::function<void()> funcDestroy, std::vector<SDL::Event> &keyPressedVec) {
    ...
    e.addComponent<SpellComponent>().addSpell(
        Spell::Bullet, std::make_unique<BulletSpell>(0.35f, [this](Vector2D initpos, Vector2D dir) { this->makeBullet(initpos, dir, CollisionLayer::Enemy); }));
    ...
    e.getComponent<SpellComponent>()->addSpell(Spell::PiercingBullet,
                                               std::make_unique<BulletSpell>(1.0f, [this](Vector2D pos, Vector2D dir) { this->makePiercingBullet(pos, dir); }));
    ...
    e.getComponent<SpellComponent>()->addSpell(Spell::SpreadBullet, std::make_unique<BulletSpell>(1.5f, [this](Vector2D pos, Vector2D dir) {
                                                   this->makeBullet(pos, dir);
                                                   this->makeBullet(
                                                       pos, Vector2D{dir.x * 0.939f - dir.y * 0.349f, dir.x * 0.349f + dir.y * 0.939f}); // we rotate 20 grads
